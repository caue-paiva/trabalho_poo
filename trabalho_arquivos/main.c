...
int main() {
    ...
    int num_remocao;
    int num_insercoes;
    bool op_sucesso, resultado_queries, resultado_cria_index, remocao_result, insercao_result;

    scanf("%d", &acao_user); // pega a ação do usuario
    switch (acao_user) {

        case 1: // Caso 1
            scanf("%s %s", nome_csv_buffer, nome_bin_buffer); // le o nome do arquivo csv e arquivo binario
            op_sucesso = csv_salva_em_registros(nome_csv_buffer, nome_bin_buffer); // tenta ler o CSV e salvar os registros no arquivo binario
            if (op_sucesso)
        ...
        case 3: // Caso 3, busca por registros
            ...
            resultado_queries = query_realizar_pesquisa(nome_bin_buffer, num_queries); // realiza a operação de pesquisa
        ...
        case 4: // funcionalidade 4, cria index e usa binario na tela nele
            ...
            resultado_cria_index = ind_cria_index(nome_bin_buffer, nome_index_buffer); // tenta criar o index
        ...
        case 5: // funcionalidade 5, remoção com auxílio do index
            ...
            remocao_result = rm_remove_registros(nome_bin_buffer, nome_index_buffer, num_remocao); // tenta remover os registros
        ...
        case 6: // funcionalidade 6, inserção e no final cria um index com os dados atualizados
            ...
            while ((d = getchar()) != '\n' && d != EOF) {} // consome a newline do começo da print
            insercao_result = ins_insere_registros(nome_bin_buffer, nome_index_buffer, num_insercoes); // tenta inserir registros
